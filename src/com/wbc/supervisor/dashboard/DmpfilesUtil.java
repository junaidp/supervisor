...
public class DmpfilesUtil {
    ...
    public static boolean isDatabaseSignatureChanged( DmpfilesRecord storedDatabase, DmpfilesRecord remoteDatabase, StringBuilder sb, Logger logger  ) {
        ...
        if ( storedDatabase.getOccurred().getTime() != remoteDatabase.getOccurred().getTime()) {
            ...
            logger.info(String.format("occurred changed from %d to %d", storedDatabase.getOccurred().getTime(), remoteDatabase.getOccurred().getTime()));
        }
        return changed;
    }
