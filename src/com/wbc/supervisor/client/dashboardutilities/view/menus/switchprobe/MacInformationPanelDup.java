...
public class MacInformationPanelDup extends SwitchProbePanel
{
	...
	//	columns.add(descIdCol);
		...
		//FILTERS
		StringFilter<MacInfo> macFilter = new StringFilter<MacInfo>(props.mac());
		NumericFilter<MacInfo, Integer> portFilter = new NumericFilter<MacInfo, Integer>(props.port(), new NumberPropertyEditor.IntegerPropertyEditor(  ) );
		StringFilter<MacInfo> decFilter = new StringFilter<MacInfo>(props.portDescription());
		NumericFilter<MacInfo, Integer> vlanFilter = new NumericFilter<MacInfo, Integer>(props.vlanNum(), new NumberPropertyEditor.IntegerPropertyEditor(  ));
		StringFilter<MacInfo> nameFilter = new StringFilter<MacInfo>(props.name());
		StringFilter<MacInfo> vendorFilter = new StringFilter<MacInfo>(props.vendor());
		StringFilter<MacInfo> ipFilter = new StringFilter<MacInfo>(props.ip());
	...
	private ArrayList<MacInfo> removeDescFromIp(List<MacInfo> data) {
		for (MacInfo macInfo : data ) {

			if(macInfo == null || macInfo.getIp() == null || macInfo.getIp().isEmpty())
				continue;
			int decIdIndex = macInfo.getIp().indexOf( " " );
			String ipAddress = macInfo.getIp().substring( 0, decIdIndex );
			macInfo.setIp(ipAddress);
		}
		return (ArrayList<MacInfo>) data;
	}
